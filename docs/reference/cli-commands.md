...
| `slides-dev` | Start dev server standalone (prefer `devenv up`) |
...
| `devenv up` | Start process-compose with TUI (foreground) |
| `devenv up -d` | Start process-compose in daemon mode (background) |
